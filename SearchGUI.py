...
class SearchGUI(Gtk.Box, object):
    ...
    def _create_widgets(self):
        ...
        self.search_label = HIGSectionLabel(_("搜索："))
        self.search_entry = Gtk.Entry()
        self.expressions_btn = HIGToggleButton(
                _("表达式 "), Gtk.STOCK_EDIT)
        ...
        self.result_title_column = Gtk.TreeViewColumn(title=_("扫描"))
        self.result_date_column = Gtk.TreeViewColumn(title=_("日期"))
...
class Criterion(Gtk.Box):
    ...
    def __init__(self, search_window, operator="keyword", argument=""):
        ...
        self.combo_entries = {"关键字": ["keyword"],
                              "配置文件名": ["profile"],
                              "目标": ["target"],
                              "选项": ["option"],
                              "日期": ["date", "after", "before"],
                              "操作系统": ["os"],
                              "端口": ["open", "scanned", "closed", "filtered",
    ...
    def new_subcriterion(self, operator="keyword", argument=""):
        if operator in self.combo_entries["日期"]:
            return DateSubcriterion(operator, argument)
        elif operator in self.combo_entries["端口"]:
...
class DirSubcriterion(Subcriterion):
    ...
    def _create_widgets(self):
        ...
        self.chooser_btn = HIGButton("选择...", Gtk.STOCK_OPEN)
...
搜索字符串会与每次扫描的完整输出进行匹配。

为了细化搜索，你可以使用 <b>运算符</b> 仅在扫描结果的 \
特定部分中进行搜索。如果你点击 <b>表达式</b> 按钮，\
可以以交互方式添加运算符，或者你也可以手动在搜索框中输入。\
大多数运算符都有对应的简写形式，已列出如下。

<b>profile: (pr:)</b> - 使用的扫描配置文件。
<b>target: (t:)</b> - 用户提供的目标，或反向 DNS（rDNS）解析结果。
<b>option: (o:)</b> - 扫描选项。
<b>date: (d:)</b> - 扫描执行的日期。可以使用 "~" 后缀进行模糊匹配。\
每增加一个 "~"，搜索范围就在日期的“前后各扩展一天”。此外，\
还可以使用 \"date:-n\" 表示“n 天前”。
<b>after: (a:)</b> - 匹配在指定日期之后执行的扫描 \
（<i>YYYY-MM-DD</i> 或 <i>-n</i>）。
<b>before (b:)</b> - 匹配在指定日期之前执行的扫描 \
（<i>YYYY-MM-DD</i> 或 <i>-n</i>）。
<b>os:</b> - 所有与操作系统相关的字段。
<b>scanned: (sp:)</b> - 如果端口属于扫描范围内，则匹配该端口。
