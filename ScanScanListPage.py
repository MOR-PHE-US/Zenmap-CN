...
class ScanScanListPage(HIGVBox):
    ...
    def __init__(self, scans_store):
        ...
        status_col = Gtk.TreeViewColumn(title=_("状态"))
        ...
        command_col = Gtk.TreeViewColumn(title=_("命令"))
        cell = Gtk.CellRendererText()
        ...
        self.append_button = HIGButton(_("追加扫描"), Gtk.STOCK_ADD)
        buttonbox.pack_start(self.append_button, False, True, 0)

        self.remove_button = HIGButton(_("删除扫描"), Gtk.STOCK_REMOVE)
        buttonbox.pack_start(self.remove_button, False, True, 0)

        self.cancel_button = HIGButton(_("取消扫描"), Gtk.STOCK_CANCEL)
