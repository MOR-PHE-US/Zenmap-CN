...
class HostDetails(HIGVBox):
    ...
    def __create_widgets(self):
        self.host_status_expander = Gtk.Expander.new(
                '<b>' + _('主机状态') + '</b>')
        self.address_expander = Gtk.Expander.new('<b>' + _('地址') + '</b>')
        self.hostnames_expander = Gtk.Expander.new('<b>' + _('主机名') + '</b>')
        self.os_expander = Gtk.Expander.new('<b>' + _('操作系统') + '</b>')
        self.portsused_expander = Gtk.Expander.new(
                '<b>' + _('使用的端口') + '</b>')
        self.osclass_expander = Gtk.Expander.new('<b>' + _('操作系统类别') + '</b>')
        self.tcp_expander = Gtk.Expander.new('<b>' + _('TCP 序列') + '</b>')
        self.ip_expander = Gtk.Expander.new('<b>' + _('IP ID 序列') + '</b>')
        self.tcpts_expander = Gtk.Expander.new(
                '<b>' + _('TCP TS 序列') + '</b>')
        self.comment_expander = Gtk.Expander.new('<b>' + _('注释') + '</b>')
        self.os_image = Gtk.Image()
        self.vulnerability_image = Gtk.Image()

        # Host Status expander
        self.host_state_label = HIGEntryLabel(_('状态:'))
        self.info_host_state_label = HIGEntryLabel(na)
        self.host_state_label.set_size_request(80, -1)
        self.info_host_state_label.set_size_request(100, -1)

        self.open_label = HIGEntryLabel(_('开放端口数:'))
        self.info_open_ports = HIGEntryLabel(na)
        self.open_label.set_size_request(80, -1)
        self.info_open_ports.set_size_request(100, -1)

        self.filtered_label = HIGEntryLabel(_('过滤端口数:'))
        self.info_filtered_label = HIGEntryLabel(na)
        self.filtered_label.set_size_request(80, -1)
        self.info_filtered_label.set_size_request(100, -1)

        self.closed_label = HIGEntryLabel(_('关闭端口数:'))
        self.info_closed_ports = HIGEntryLabel(na)
        self.closed_label.set_size_request(80, -1)
        self.info_closed_ports.set_size_request(100, -1)

        self.scanned_label = HIGEntryLabel(_('扫描端口数:'))
        self.info_scanned_label = HIGEntryLabel(na)
        self.scanned_label.set_size_request(80, -1)
        self.info_scanned_label.set_size_request(100, -1)

        self.uptime_label = HIGEntryLabel(_('在线时间:'))
        self.info_uptime_label = HIGEntryLabel(na)
        self.uptime_label.set_size_request(80, -1)
        self.info_uptime_label.set_size_request(100, -1)

        self.lastboot_label = HIGEntryLabel(_('上次启动:'))
        self.info_lastboot_label = HIGEntryLabel(na)
        self.lastboot_label.set_size_request(80, -1)
        self.info_lastboot_label.set_size_request(100, -1)

        # Addresses expander
        self.ipv4_label = HIGEntryLabel('IPv4:')
        self.info_ipv4_label = HIGEntryLabel(na)
        self.ipv4_label.set_size_request(80, -1)
        self.info_ipv4_label.set_size_request(100, -1)

        self.ipv6_label = HIGEntryLabel('IPv6:')
        self.info_ipv6_label = HIGEntryLabel(na)
        self.ipv6_label.set_size_request(80, -1)
        self.info_ipv6_label.set_size_request(100, -1)

        self.mac_label = HIGEntryLabel('MAC:')
        self.info_mac_label = HIGEntryLabel(na)
        self.mac_label.set_size_request(80, -1)
        self.info_mac_label.set_size_request(150, -1)

        self.vendor_label = HIGEntryLabel(_('制造商:'))
        self.info_vendor_label = HIGEntryLabel(na)
        self.vendor_label.set_size_request(80, -1)
        self.info_vendor_label.set_size_request(100, -1)
    ...
    def set_hostnames(self, hostname):
        if hostname:
            ...
            for h in hostname:
                ...
                label1 = HIGEntryLabel(_('名称 - 类型:'))
                label2 = HIGEntryLabel(name + ' - ' + type)
                label1.set_size_request(80, -1)
                label2.set_size_request(300, -1)

                table.attach(label1, 0, 1, y1, y2)
                table.attach(label2, 1, 2, y1, y2)
                table.set_col_spacing(0, 80)
                table.set_col_spacing(1, 100)
                y1 += 1
                y2 += 1
    ...
    def set_os(self, os):
        if os:
            ...
            if 'accuracy' in os:
                ...
                progress.set_text(_('暂无'))

            
            label1 = HIGEntryLabel(_('名称:'))
            label2 = HIGEntryLabel(os['name'])
            label1.set_size_request(80, -1)
            label2.set_size_request(300, -1)
            table.attach(label1, 0, 1, 0, 1)
            table.attach(label2, 1, 2, 0, 1)

            label1 = HIGEntryLabel(_('准确性:'))
            label1.set_size_request(80, -1)
    ...
    def set_ports_used(self, ports):
        ...
        for p in ports:
            label1 = HIGEntryLabel(
                _('端口-协议-状态:'))
            label2 = HIGEntryLabel(
    ...
    def set_osclass(self, osclass):
        if osclass:
            ...
            label1 = HIGEntryLabel(_('类型'))
            label2 = HIGEntryLabel(_('制造商'))
            label3 = HIGEntryLabel(_('OS 系列'))
            label4 = HIGEntryLabel(_('OS 版本'))
            label5 = HIGEntryLabel(_('准确性'))
            label1.set_size_request(70, -1)
            label2.set_size_request(70, -1)
            label3.set_size_request(70, -1)
            label4.set_size_request(70, -1)
            label5.set_size_request(70, -1)
            table.attach(label1, 0, 1, 0, 1)
            table.attach(label2, 1, 2, 0, 1)
            table.attach(label3, 2, 3, 0, 1)
            table.attach(label4, 3, 4, 0, 1)
            table.attach(label5, 4, 5, 0, 1)

            y1 = 1
            y2 = 2

            for o in osclass:
                label1 = HIGEntryLabel(o['type'])
                label2 = HIGEntryLabel(o['vendor'])
                label3 = HIGEntryLabel(o['osfamily'])
                label4 = HIGEntryLabel(o['osgen'])
                label1.set_size_request(70, -1)
                label2.set_size_request(70, -1)
                label3.set_size_request(70, -1)
                label4.set_size_request(70, -1)
                table.attach(label1, 0, 1, y1, y2)
                table.attach(label2, 1, 2, y1, y2)
                table.attach(label3, 2, 3, y1, y2)
                table.attach(label4, 3, 4, y1, y2)
    ...
    def set_tcpseq(self, tcpseq):
        if tcpseq:
            ...
            label1 = HIGEntryLabel(_('难度'))
            label2 = HIGEntryLabel(tcpseq['difficulty'])
            label1.set_size_request(80, -1)
            label2.set_size_request(200, -1)
            table.attach(label1, 0, 1, 1, 2)
            table.attach(label2, 1, 2, 1, 2)

            label1 = HIGEntryLabel(_('索引'))
            label2 = HIGEntryLabel(tcpseq['index'])
            label1.set_size_request(80, -1)
            label2.set_size_request(200, -1)
            table.attach(label1, 0, 1, 2, 3)
            table.attach(label2, 1, 2, 2, 3)

            label1 = HIGEntryLabel(_('值'))
            label1.set_size_request(80, -1)
    ...
    def set_ipseq(self, ipseq):
        if ipseq:
            ...
            label1 = HIGEntryLabel(_('类别:'))
            label2 = HIGEntryLabel(ipseq['class'])
            label1.set_size_request(80, -1)
            label2.set_size_request(200, -1)
            table.attach(label1, 0, 1, 0, 1)
            table.attach(label2, 1, 2, 0, 1)
    ...
    def set_tcptsseq(self, tcptsseq):
        if tcptsseq:
            ...
            label1 = HIGEntryLabel(_('类别:'))
            label2 = HIGEntryLabel(tcptsseq['class'])
            label1.set_size_request(80, -1)
            label2.set_size_request(200, -1)
            table.attach(label1, 0, 1, 0, 1)
            table.attach(label2, 1, 2, 0, 1)
