...
class ProfileEditor(HIGWindow):
    ...
    def __create_widgets(self):
        ...
        self.scan_button = HIGButton(_("扫描"))
        ...
        self.profile_info_label = HIGSectionLabel(_('配置信息'))
        self.profile_name_label = HIGEntryLabel(_('配置文件名'))
        ...
        self.profile_description_label = HIGEntryLabel(_('描述'))
        self.profile_description_label.set_size_request(40, -1)
        ...
        self.cancel_button = HIGButton(_("取消"), stock=Gtk.STOCK_CANCEL)
        self.cancel_button.connect('clicked', self.exit)

        self.delete_button = HIGButton(_("删除"), stock=Gtk.STOCK_DELETE)
        self.delete_button.connect('clicked', self.delete_profile)

        self.save_button = HIGButton(_("保存更改"), stock=Gtk.STOCK_SAVE)
        self.save_button.connect('clicked', self.save_profile)

        ###
        self.help_vbox = HIGVBox()
        self.help_label = HIGSectionLabel(_('提示'))
    ...
    def __pack_widgets(self):
        ...
        self.notebook.append_page(
                self.profile_info_vbox, Gtk.Label.new(_('属性')))
    ...
    def delete_profile(self, widget=None, extra=None):
        if self.deletable:
            ...
            alert = HIGEntryLabel('<b>' + _("删除配置") + '</b>')
