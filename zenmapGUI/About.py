...
class About(HIGDialog):
    ...
    def __init__(self):
        ...
        entry = _program_entry(NMAP_DISPLAY_NAME, NMAP_WEB_SITE, _(
            "%s 是一款免费开源的网络探测和安全检测工具。") % NMAP_DISPLAY_NAME)
        ...
        entry = _program_entry(UMIT_DISPLAY_NAME, UMIT_WEB_SITE, _(
            "%s 是 %s 的图形化界面，它是参与 Nmap 和 Google 举办的夏季编程项目时开发出来的。") % (UMIT_DISPLAY_NAME, NMAP_DISPLAY_NAME))
        button = Gtk.Button.new_with_label(_("%s 软件致谢") % UMIT_DISPLAY_NAME)
        button.connect("clicked", self._show_umit_credits)
        entry.hbox.pack_start(button, False, True, 0)
        self.vbox.pack_start(entry, True, True, 0)

        entry = _program_entry("汉化制作：Morpheus", "https://www.suidx.com")
        self.vbox.pack_start(entry, True, True, 0)

        self.vbox.show_all()

        close_button = self.add_button(_("关闭"), Gtk.ResponseType.CANCEL)
