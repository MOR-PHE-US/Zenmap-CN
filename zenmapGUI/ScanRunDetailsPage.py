...
class ScanRunDetailsPage(HIGVBox):
    def __init__(self, scan):
        ...
        # Command info
        self.command_label = HIGEntryLabel(_('命令：'))
        self.info_command_label = HIGEntryLabel(na)

        self.nmap_version_label = HIGEntryLabel(_('Nmap 版本：'))
        self.info_nmap_version_label = HIGEntryLabel(na)

        self.verbose_label = HIGEntryLabel(_('详细程度：'))
        self.info_verbose_label = HIGEntryLabel(na)

        self.debug_label = HIGEntryLabel(_('调试级别：'))
        ...
        # General info:
        self.start_label = HIGEntryLabel(_('开始时间：'))
        self.info_start_label = HIGEntryLabel(na)
        self.start_label.set_size_request(80, -1)
        self.info_start_label.set_size_request(200, -1)

        self.finished_label = HIGEntryLabel(_('完成时间：'))
        self.info_finished_label = HIGEntryLabel(na)
        self.start_label.set_size_request(80, -1)
        self.info_start_label.set_size_request(200, -1)

        self.host_up_label = HIGEntryLabel(_('在线主机：'))
        self.info_hosts_up_label = HIGEntryLabel(na)
        self.start_label.set_size_request(80, -1)
        self.info_start_label.set_size_request(200, -1)

        self.host_down_label = HIGEntryLabel(_('离线主机：'))
        self.info_hosts_down_label = HIGEntryLabel(na)
        self.start_label.set_size_request(80, -1)
        self.info_start_label.set_size_request(200, -1)

        self.host_scanned_label = HIGEntryLabel(_('扫描的主机：'))
        self.info_hosts_scanned_label = HIGEntryLabel(na)
        self.start_label.set_size_request(80, -1)
        self.info_start_label.set_size_request(200, -1)

        self.open_label = HIGEntryLabel(_('开放端口：'))
        self.info_open_label = HIGEntryLabel(na)
        self.start_label.set_size_request(80, -1)
        self.info_start_label.set_size_request(200, -1)

        self.filtered_label = HIGEntryLabel(_('过滤端口：'))
        self.info_filtered_label = HIGEntryLabel(na)
        self.start_label.set_size_request(80, -1)
        self.info_start_label.set_size_request(200, -1)

        self.closed_label = HIGEntryLabel(_('关闭端口:'))
    ...
    def _set_from_scan(self, scan):
        ...
        for scaninfo in scan.get_scaninfo():
            exp = Gtk.Expander.new('<b>%s - %s</b>' % (
                _('扫描信息'), scaninfo['type'].capitalize()))
    ...
    def make_scaninfo_display(self, scaninfo):
        ...
        scan_type_label = HIGEntryLabel(_('扫描类型：'))
        scan_type_label.set_size_request(80, -1)
        table.attach(scan_type_label, 0, 1, 0, 1)
        table.attach(HIGEntryLabel(scaninfo['type']), 1, 2, 0, 1)

        protocol_label = HIGEntryLabel(_('协议：'))
        protocol_label.set_size_request(80, -1)
        table.attach(protocol_label, 0, 1, 1, 2)
        table.attach(HIGEntryLabel(scaninfo['protocol']), 1, 2, 1, 2)

        scan_ports_label = HIGEntryLabel(_('扫描端口数：'))
        scan_ports_label.set_size_request(80, -1)
        table.attach(scan_ports_label, 0, 1, 2, 3)
        table.attach(HIGEntryLabel(scaninfo['numservices']), 1, 2, 2, 3)

        services_label = HIGEntryLabel(_('服务：'))
        services_label.set_size_request(80, -1)
        table.attach(services_label, 0, 1, 3, 4)
        table.attach(
