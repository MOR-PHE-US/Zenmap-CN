...
<big><b>如何报告 bug</b></big>

就像它们的作者一样，%(nmap)s 和 %(app)s 也并非完美无缺。但你可以通过提交
错误报告，甚至编写补丁来帮助改进它。如果 %(nmap)s 的行为与你的预期不符，
请先升级到 <b>%(nmap_web)s</b> 提供的最新版本。如果问题仍然存在，请先进行
一些调查，以确定该问题是否已经被发现并解决。可以尝试在 Google 中搜索
错误信息，或浏览 https://seclists.org 上的 nmap-dev 邮件列表归档。
同时也请阅读完整的手册页面。如果这些方法都没有结果，请在
<b>https://issues.nmap.org</b> 提交一份错误报告。请尽可能详细地提供你已经
了解的所有问题信息，以及你正在运行的 Nmap 版本和其所运行的操作系统版本。

用于修复错误的代码补丁甚至比错误报告更受欢迎。关于如何创建包含你修改内容的
补丁文件的基本说明，可在以下地址获取：
