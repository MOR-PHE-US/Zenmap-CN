...
class AllFilesFileFilter(Gtk.FileFilter):
    def __init__(self):
        ...
        self.set_name(_("所有文件 (%s)") % pattern)
...
class ResultsFileFilter(Gtk.FileFilter):
    def __init__(self):
        ...
        for pattern in patterns:
            self.add_pattern(pattern)
        self.set_name(_("Nmap XML文件 (%s)") % ", ".join(patterns))
...
class ScriptFileFilter(Gtk.FileFilter):
    def __init__(self):
        ...
        for pattern in patterns:
            self.add_pattern(pattern)
        self.set_name(_("NSE 脚本 (%s)") % ", ".join(patterns))
...
class ResultsFileChooserDialog(Gtk.FileChooserDialog):
    def __init__(self, title="", parent=None,
                 action=Gtk.FileChooserAction.OPEN,
                 buttons=("取消", Gtk.ResponseType.CANCEL,
                          "打开目录", RESPONSE_OPEN_DIRECTORY,
                          "打开", Gtk.ResponseType.OK), backend=None):
...
class SaveResultsFileChooserDialog(Gtk.FileChooserDialog):
    TYPES = (
        (_("按扩展名"), None, None),
        (_("Nmap XML 格式 (.xml)"), "xml", ".xml"),
        (_("Nmap text 格式 (.nmap)"), "text", ".nmap"),
    )
    ...
    def __init__(self, title="", parent=None,
                 ...
                 buttons=(_("取消"), Gtk.ResponseType.CANCEL,
                          _("保存"), Gtk.ResponseType.OK), backend=None):
        ...
        hbox.pack_end(Gtk.Label.new(_("选择文件类型：")), False, True, 0)
        hbox.show_all()
