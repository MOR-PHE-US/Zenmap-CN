...
class ScanWindow(UmitScanWindow):
    ...
    def _save_scan_results_cb(self, widget):
        ...
        if num_scans_running > 0:
            if num_scans_running == 1:
                ...
                text = _("有 %u 个扫描仍在运行。等到他们完成，然后保存。") % num_scans_running
            alert = HIGAlertDialog(message_format=_('扫描正在运行'),
                                   secondary_text=text)
            alert.run()
            alert.destroy()
            return
        ...
        self._save_results_filechooser_dialog = \
            SaveResultsFileChooserDialog(title=_('保存扫描'))
    ...
    def _save_to_directory_cb(self, widget):
        if self.scan_interface.empty:
            alert = HIGAlertDialog(message_format=_('没什么可保存的'),
                                   secondary_text=_('\
此扫描尚未运行。先按“扫描”按钮开始扫描。'))
            alert.run()
            alert.destroy()
            return
        num_scans_running = self.scan_interface.num_scans_running()
    ...
    def _save_all(self, scan_interface, directory):
        ...
        try:
            ...
            alert = HIGAlertDialog(message_format=_('无法保存文件'),
    ...
    def _exit_cb(self, *args):
        ...
        if self.scan_interface.changed:
            ...
            dialog = HIGDialog(
                    buttons=(_('直接关闭'),
                        Gtk.ResponseType.CLOSE, _('取消'),
                        Gtk.ResponseType.CANCEL))

            alert = HIGEntryLabel('<b>%s</b>' % _("有更改未保存"))

            text = HIGEntryLabel(_("当前扫描有未保存的更改.\n"
                "你要怎么做?"))
            hbox = HIGHBox()
        ...
        if config_parser.failed:
            alert = HIGAlertDialog(
                    message_format=_("无法保存 Zenmap 配置"),
