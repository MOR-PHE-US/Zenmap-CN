...
class LegendWindow(Gtk.Window):
    ...
    def draw_event_handler(self, widget, graphic_context):
        ...
        """
        x, y = 45, 20
        draw_heading(graphic_context, x, y, _("主机"))
        ...
        graphic_context.select_font_face("SimSun", cairo.FONT_SLANT_NORMAL, cairo.FONT_WEIGHT_NORMAL)
        graphic_context.set_font_size(12)
        graphic_context.show_text(_("主机是路由器、交换机或无线接入点"))

        # connections between hosts
        y += 30
        draw_heading(graphic_context, x, y, _("追踪路由连接"))

        y += 20
        graphic_context.move_to(x, y)
        graphic_context.select_font_face("SimSun", cairo.FONT_SLANT_NORMAL, cairo.FONT_WEIGHT_NORMAL)
        graphic_context.set_font_size(12)
        graphic_context.show_text(_("线条越粗表示往返时间越高"))
        ...
        y += 30
        draw_heading(graphic_context, x, y, _("额外的主机图标"))

        # router image
        y += 20
        draw_pixmap(graphic_context, x, y, "router", _("路由器"))

        # switch image
        y += 20
        draw_pixmap(graphic_context, x, y, "switch", _("交换机"))
        ...
        # firewall image
        y += 20
        draw_pixmap(graphic_context, x, y, "firewall", _("防火墙"))
